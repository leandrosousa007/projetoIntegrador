 ...
 * @author leand
 */
public class AlunoTurmaControle {
    ...
    public AlunoTurma consultarAlunoTurmaPorCodigo(int codAlunoTurma) {
        ...
        try (Connection conn = conexao.conectar();
            ...
            Logger.getLogger(AlunoTurmaControle.class.getName()).log(Level.SEVERE, null, ex);
        }
        return alunoTurma;
    }
...
public String inserirAlunoTurma(AlunoTurma alunoTurma) {
    ...
    try {
        ...
        if (linhasAfetadas > 0) {
            ...
            resultado = "Nenhum registro inserido.";
        }
    } catch (SQLException ex) {
        resultado = "Erro ao inserir registro: " + ex.getMessage();
    } finally {
   ...
   public String alterarAlunoTurma(AlunoTurma alunoTurma) {
    ...
    try {
        ...
        if (linhasAfetadas > 0) {
            ...
            resultado = "Nenhum registro alterado";
        }
    } catch (SQLException ex) {
        resultado = "Erro ao alterar: " + ex.getMessage();
    } finally {
        // Fechamos os recursos em um bloco finally para garantir que sejam fechados, mesmo em caso de exceção
        if (stm != null) {
            try {
    ...
    try {
        ...
        int linhasAfetadas = stm.executeUpdate();
        if (linhasAfetadas > 0) {
            resultado = "Registro removido com sucesso.";
        } else {
            resultado = "Nenhum registro removido.";
        }
    } catch (SQLException ex) {
        resultado = "Erro ao remover registro: " + ex.getMessage();
    } finally {
        // Fechando os recursos
        if (stm != null) {
            try {
